...
$inner_template1 		= 'templates/inner_categories_videos.html';
$edit_category 		= 0;
$edit_sub_category 	= 0;
...
if(isset($_POST['new_sub_category']) && isset($_POST['main_category'])) {
	...
	//check if already exists
	//TODO CHECK IF EXISTS ONLY IF PARENT ID IS NOT THE SAME ?? AND FIX DUP ERROR MESSAGE
	if($title != ''){
		$sql = "SELECT sub_channel_id from sub_channels WHERE sub_channel_name = '$title' AND parent_channel_id = '$parent_channel_id'";
		if(@mysql_num_rows(@mysql_query($sql)) >0){
			$show_notification =1;
			$message = $config['duplicate_item'];
			$proceed = false;
...
//set inner template
$inner_template1 = 'templates/inner_categories_videos_sub.html';
