...
function managevideo($vid,$manage_type) {
	...
	if($manage_type == 'delete') {
		$sql		= "SELECT * FROM videos WHERE indexer = $video_id";
		$query 	= mysql_query($sql);
		while ($result = mysql_fetch_array($query)) {
			$file_name_no_extension	= $result['video_id'];
      		$v_type			= $result['type'];
      		$flv_video			= $base_path.'/uploads/'.$file_name_no_extension.'.'.$v_type;
			unlink($flv_video);
	      	$thumb 			= $base_path.'/uploads/thumbs/'.$file_name_no_extension.'.jpg';
      		$player_thumb		= $base_path.'/uploads/player_thumbs/'.$file_name_no_extension.'.jpg';
	      	unlink($thumb);
      		unlink($player_thumb);
