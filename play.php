...
if ( $proceed == true ) {
	...
	$video_thumb	= $result['video_id'].'.jpg';
    	...
    	$og_type		= 'video';
	$og_media_url	= $seo_video_url;
	$og_video		= "$base_url/player.swf?flvsource=$base_url/uploads/$video_play&amp;preview_image=$og_image_url&amp;autoplay=false";
	$og_image_url	= $config['site_base_url'].'/uploads/player_thumbs/'.$video_thumb;
	$og_description	= $description;
      ...
      //---------------------------------------------------------------
    	...
    	if (!$_SESSION['time_stamp']) {
		$views_counter 	= $number_of_views + 1;
    		$sql 			= "UPDATE videos SET number_of_views = $views_counter WHERE indexer = $vid";
    		$query		= @mysql_query($sql);
		session_register('time_stamp');
		$_SESSION['time_stamp'] = time();
	}

	if ($_SESSION['vid_id'] != $video_id ) {
		$views_counter 	= $number_of_views + 1;
    		$sql 			= "UPDATE videos SET number_of_views = $views_counter WHERE indexer = $vid";
    		$query		= @mysql_query($sql);
    		session_register('vid_id');
		$_SESSION['vid_id'] = $video_id;
	}


	/*
	$check_time = time() - $_SESSION['time_stamp'];
    	if( $check_time > 86400 ) {
    		$views_counter 	= $number_of_views + 1;
    		$sql 			= "UPDATE videos SET number_of_views = $views_counter WHERE indexer = $vid";
    		$query		= @mysql_query($sql);
