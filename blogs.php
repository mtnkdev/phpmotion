...
if ($enable_blogs == 'no' ) header("Location: " . "index.php");

$is_allowed	= $_SESSION['user_group'];
...
if ($procede == false && $blog_submitted == 'yes') {
	...
	while ( $result = @mysql_fetch_array($query) ) $fields_all .= '<option value="'.$result['category_id'].'">'.$result['category_name'].'</option>';
...
if ( $procede == true && $blog_submitted == 'yes' ) {
	...
	$blog_story		= $_POST['FCKeditor1'];
	$checks 	= array('http://', 'https://', 'http://www', 'https://www', 'www.');
	foreach($checks as $ng) {
		$found = strpos($blog_story, $ng);
		
		if(strpos($blog_story, $ng) != 0) {
			if ( $is_allowed == 'member' ) {
				$sql_d = "DELETE FROM member_profile WHERE user_ip = '$user_ip'";
				mysql_query($sql_d);
				$sql_b = "DELETE FROM blogs WHERE blog_owner = '$blog_owner'";
				mysql_query($sql_b);
				die();
			}	
		}
	...
	$update_blogs_sql	= "UPDATE blog_categories SET has_blogs = 'yes' WHERE category_id = $insert_category_id";
...
if ( $action != '' ) {
	...
	$sql				= "SELECT indexer FROM blogs WHERE user_id = '$user_id'";
	$member_blog_total	= @mysql_num_rows(mysql_query($sql));

	if ( $member_blog_total >= $blog_max_limits  && $is_allowed == 'member' ) exit();
	...
	if ( $action == 'write' ) {
		...
		if ( $member_blog_total >= $blog_max_limits  && $is_allowed == 'member' ) {
			$show_block = 3;
