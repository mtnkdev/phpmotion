...
$vid 		= $postedValue[1];
$comment_ck	= $_POST['FCKeditor1'];

$checks 	= array('http://', 'https://', 'http://www', 'https://www', 'www.');
foreach($checks as $ng) {
	$found = strpos($comment_ck, $ng);
	if(strpos($comment_ck, $ng) != 0) {
		echo '<p style="text-align:center;color:#FF4242;font-family:arial"><b>HTML is not allowed</b></p>';
		die();
	}
}

if( strlen($comment_ck) != strlen(html_entity_decode($comment_ck)) ) {
	echo '<p style="text-align:center;color:#FF4242;font-family:arial"><b>HTML is not allowed</b></p>';
	die();
}
...
$sql = "INSERT into videocomments (by_id, by_username, video_id, comments, todays_date) VALUES ($user_id, '$user_name', $vid, '$comments', NOW())";
mysql_query($sql);

	echo '<p align="center"><font color="#009933" face="Arial"><b>'.$config['video_comments_success'].' Your IP has been recorded!</b></font>';
