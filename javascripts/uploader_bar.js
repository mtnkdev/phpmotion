...
function smoothCedricStatus(){
	...
	if(get_data_loop){ setTimeout("smoothCedricStatus()", info_time_width); }
}
...
function smoothCedricBytes(){
	...
	if(get_data_loop){ setTimeout("smoothCedricBytes()", info_time_bytes); }
}
...
function getProgressStatus(){
	...
	if(get_data_loop){ setTimeout("getProgressStatus()", get_status_speed); }
}
...
function iniProgressResponse(req){
	if(req.readyState == 4){
		if(req.status == 200){
			...
			document.getElementById('progress_info_2').style.display="block";
			document.getElementById('progress_info').innerHTML = "Upload In Progress";
