...
function seo_title($title) {
	...
	$title_seo = str_replace('"', "", $title_seo);
	$title_seo = str_replace("'", "", $title_seo);
	$title_seo = str_replace("&", "and", $title_seo);
	$title_seo = str_replace("?", "", $title_seo);
...
function media_views_tracker($id, $type) {  //type should be name of media tables in dbase
	...
	if($id !='' && $type !='') {
		if ($type =='videos' || $type =='audios' || $type =='blogs' || $type =='images') {
...
function admin_notify() {
	...
	//get admin email address for global notifications
	$sql				= "SELECT user_id, email_address FROM member_profile WHERE user_group = 'admin' ORDER BY user_id ASC";
	$query 			= mysql_query($sql);
