...
$sql_c		= "SELECT * FROM member_profile WHERE user_id = $member_id";
$query_c		= @mysql_query($sql_c);
$result_c 		= @mysql_fetch_array($query_c);
$member_username 	= $result_c['user_name'];

$sql_a		= "SELECT * FROM member_profile WHERE user_group = 'admin'";
$query_a		= @mysql_query($sql_a);
$result_a 		= @mysql_fetch_array($query_a);
$member_id_a	= $result_a['user_id'];


if ($member_username == '') {
    	$sql 			= "UPDATE audios SET user_id = $member_id_a WHERE indexer = $audio";
    	$query		= @mysql_query($sql);
    	$sql			= "SELECT * FROM audios WHERE indexer = $audio AND approved = 'yes'";
$query 		= @mysql_query($sql);
$result 		= @mysql_fetch_array($query);
